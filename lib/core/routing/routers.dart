class Routers {
  static const String loginRoute = '/';
  static const String registerRoute = '/register';

  static const String homeRoute = '/homeRoute';
  
  static const String addCustomerRoute = '/add-customer';
  static const String editCustomerRoute = '/editCustomer';


  static const String productRoute = '/productsRoute';
  static const String purchaseInvoicesList = '/purchase-invoices-list';
  static const String addProductRoute = '/add-product';
  static const String productDetailsRoute = '/product-details';
  static const String customersRoute = '/customers-details';
  static const String suppliersRoute = '/suppliers-details';
  static const String purchaseInvoiceRoute = '/purchase-invoice';
  static const String salesInvoiceRoute = '/sales-invoice';
  static const String salesInvoicesList = '/sales-invoices-list';
  static const String quickSaleRoute = '/quick-sale';
  static const String deferredAccountsRoute = '/deferred-accounts';
  static const String reportRoute = '/reports';
  static const String backup = '/backup';
}
