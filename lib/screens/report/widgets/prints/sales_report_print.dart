...
class SalesReportPrintWidget extends StatelessWidget {
  ...
  Widget _buildReportContent() {
    ...
    // 🔥 البيانات بتيجي List<MapEntry>
    final topProductsData = data['topProducts'];
    final topProducts = (topProductsData is List) ? topProductsData.cast<MapEntry>() : <MapEntry>[];
      ...
      child: Column(
        ...
        children: [
          Center(
            child: Column(
              children: [
                const Text('تقرير المبيعات 📈', style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: Colors.black87)),
          ...
          _buildStatRow('إجمالي المبيعات', currencyFormat.format(totalSales), Colors.green),
          const Divider(),
          _buildStatRow('عدد الفواتير', '$totalTransactions', Colors.blue),
          const Divider(),
          _buildStatRow('متوسط الفاتورة', currencyFormat.format(averageOrderValue), Colors.orange),
          const SizedBox(height: 32),
          const Text('أعلى المنتجات مبيعاً:', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.black87)),
          const SizedBox(height: 16),
          _buildTopProductsTable(topProducts, revenue, currencyFormat),
        ],
      ),
    );
  }
  ...
  Widget _buildTopProductsTable(List<MapEntry> topProducts, Map revenue, NumberFormat currencyFormat) {
          ...
          return pw.Column(
            ...
            children: [
              pw.Center(
                child: pw.Column(
                  children: [
                    pw.Text('تقرير المبيعات', style: pw.TextStyle(fontSize: 28, fontWeight: pw.FontWeight.bold)),
              ...
              _pdfStatRow('إجمالي المبيعات', currencyFormat.format(totalSales)),
              pw.Divider(),
              _pdfStatRow('عدد الفواتير', '$totalTransactions'),
              pw.Divider(),
              _pdfStatRow('متوسط الفاتورة', currencyFormat.format(averageOrderValue)),
              pw.SizedBox(height: 24),
              pw.Text('أعلى المنتجات مبيعاً:', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 16)),
                  ...
                  children: [
                    pw.TableRow(
                      ...
                      children: [
                        ...
                        _pdfTableCell('الإيرادات', bold: true),
                      ],
                    ),
                    ...topProducts.take(10).map((product) {
                      final productName = product.key?.toString() ?? 'غير محدد';
                      ...
                      return pw.TableRow(
                        children: [
                          ...
                          _pdfTableCell(currencyFormat.format(productRevenue)),
                        ],
                      );
                    }),
                  ],
